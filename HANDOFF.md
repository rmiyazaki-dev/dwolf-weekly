...
4. ダッシュボード: KPI集計(受注額・**見積合計**・未請求額・入金済等)、**期・月次の受注/売上予測**、分類別/担当別グラフ、**営業担当別の受注額集計・フライト担当別の売上合計集計**(未設定は「未分類」にまとめる)、今月実施一覧。**分類別/営業担当別/フライト担当別は受注額(orderAmount>0)が確定している案件のみ集計**(見積のみの案件は含めない。担当別は従来どおり進行中案件全件+人日)。**各グラフの項目名をクリックすると案件管理タブへその条件で絞り込んで遷移**(「絞り込み解除」で戻せる)
5. 連絡事項: 掲示板+既読管理
