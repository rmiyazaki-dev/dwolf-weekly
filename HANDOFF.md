...
4. **見積→請求書は1つの案件に集約する**。MFのAPIには見積書と請求書を直接つなぐフィールドが無いため、「既に見積が紐付いていて・まだ請求書は紐付いておらず・取引先名が一致する案件」を金額の一致に関わらず優先候補にすることで、見積と請求書が別々の案件・別々の要確認リスト項目に「重複」するのを防いでいる(`findQuoteLinkedCandidates`)
5. **見積額(quoteAmount)=見積書の金額、受注額(orderAmount)=請求書の金額(税込)** として別フィールドで保持する(金額が違っても両方登録する)。自動突合・手動紐付けのどちらでも書き込まれる
6. 要確認リストで「見送り」した項目も、**「見送り済みも表示」トグルで再度呼び出して紐付け直せる**(以前は見送ると二度と操作できなかった)。また各項目に「新規案件として登録」ボタンがあり、案件モーダルを取引先名・金額をプリフィルした状態で開いて、顧客(マスター選択 or 直接入力)も含めてその場で新規登録できる
