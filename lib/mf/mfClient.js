...
/* 取引先一覧の全件取得(公式クライアントと同じ /partners?page=N&per_page=100。from/to不要) */
async function listPartners(accessToken, { maxPages = 20, perPage = 100 } = {}) {
  const items = [];
  for (let page = 1; page <= maxPages; page++) {
    const json = await mfGet(accessToken, `/partners?page=${page}&per_page=${perPage}`);
    const pageItems = json.data || [];
    items.push(...pageItems);
    const pagination = json.pagination || {};
    if (!pagination.total_pages || page >= pagination.total_pages || !pageItems.length) break;
