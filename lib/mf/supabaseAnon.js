/*
 * cases / case_history / audit_log / mf_review_queue / kv 用。
 * 既存 index.html と同じ SUPABASE_ANON_KEY で PostgREST を叩く薄いラッパー。
 */
...
async function anonFetch(path, opts = {}) {
  ...
  if (!res.ok) {
    ...
    throw new Error(`Supabase(anon) ${res.status} ${path}: ${text.slice(0, 300)}`);
  }
...
async function queueReviewItem(item) {
  await anonFetch("mf_review_queue", {
    method: "POST",
