...
const {calendarConfiguration, calendarAuthorizeUrl, refreshCalendarAccess, calendarIdentity, loadCalendarToken, saveCalendarToken} = require('./calendar');
function configuration(env = process.env) {
  const calendar = calendarConfiguration(env);
  let routing; try { routing = teamsRouting(env); } catch { /* 通知先の設定不備では送信しない。 */ }
  return {calendar: calendar.calendar, teams: !!(routing && env.ONEONONE_TEAMS_WEBHOOK && env.ONEONONE_TEAMS_SECRET && env.ONEONONE_APP_ORIGIN), ai: !!(env.ONEONONE_AI_APPROVED === 'true' && env.ONEONONE_AZURE_ENDPOINT && env.ONEONONE_AZURE_KEY && env.ONEONONE_AZURE_MODEL), calendarAccount: calendar.calendarAccount, teamsAccount: routing?.manager || null, teamsFlowUrl: flowHistoryUrl(env.ONEONONE_TEAMS_FLOW_URL)};
}
function createIntegrations(env = process.env, request = fetch, store) {
  ...
  async function graphClient() {
    ...
    const refreshToken = await loadCalendarToken(store, env);
    if (!refreshToken) throw error(503, 'コプロスの予定表連携が未接続です。');
    const auth = await refreshCalendarAccess(env, refreshToken, request);
    if (auth.refresh_token) await saveCalendarToken(store, env, auth.refresh_token);
    ...
    const identity = await calendarIdentity(auth.access_token, request);
    if (identity !== CALENDAR_MAIL) throw error(403, '指定のコプロス予定表を確認できません。');
    const cal = await graph('/calendar?$select=id,owner,canEdit');
    if ((cal.owner?.address && cal.owner.address.toLowerCase() !== CALENDAR_MAIL) || !cal.canEdit) throw error(403, '指定のコプロス予定表を確認できません。');
    return graph;
  }
  ...
  async function available(graph, start, excludeId) {
    ...
    const view = await graph(`/calendar/calendarView?startDateTime=${encodeURIComponent(begin)}&endDateTime=${encodeURIComponent(end)}&$select=id,showAs,isCancelled&$top=100`);
  ...
  async function calendar(job, member) {
    ...
    const eventBase = '/calendar/events';
    if (p.op === 'cancel') {
    ...
    if (!prev.eventId) {
      ...
      const existing = await graph(`/calendar/calendarView?startDateTime=${encodeURIComponent(begin)}&endDateTime=${encodeURIComponent(end)}&$select=id,transactionId,isCancelled&$top=100`);
  ...
  async function calendarReady() {
    ...
    try { return !!await loadCalendarToken(store, env); } catch { return false; }
  }
  ...
  return {config, availability, calendar, calendarReady, calendarAuthorizeUrl: actor => calendarAuthorizeUrl(env, actor.user_id), teams, generate};
}
