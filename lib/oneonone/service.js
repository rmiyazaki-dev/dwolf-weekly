...
function createService(store, integrations, env = process.env) {
  ...
  async function bootstrap(actor) {
    ...
    return {actor: {user_id: actor.user_id, name: actor.name, role: actor.role}, members: members.map(m => ({user_id: m.user_id, name: m.name, role: m.role})), cycle, cycles, records, consultations, config: actor.role === 'manager' ? integrations.config : {ai: integrations.config.ai}, alternateContact: env.ONEONONE_ALTERNATE_CONTACT || '', retention: env.ONEONONE_RETENTION_NOTICE || '保存期間は運用開始前に決定します。', available: true};
  }
  ...
  async function dispatch(actor, id) {
    ...
    try {
      ...
      if (!integrations.config[job.kind]) { state = 'not_configured'; result = {message: '連携未設定'}; }
      else {
  ...
  async function tick(now=new Date()) {
    ...
    for(const c of cycles){if(!c.opens_at||c.opens_at>today)continue;const records=await store.get('oneonone_records',`cycle=eq.${c.id}&submitted_version=eq.0`);for(const r of records){const phase=c.deadline&&c.deadline<today?'overdue':'request';await store.insert('oneonone_outbox',{record_id:r.id,kind:'teams',dedupe:`reminder:${r.id}:${phase}`,payload:{type:'reminder'}},true);queued++;}}
    return {queued};
  }
